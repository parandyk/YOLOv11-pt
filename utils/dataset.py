...
class Dataset(data.Dataset):
    ...
    def collate_fn(batch): #original
        ...
        if "labels" not in targets:
            ...
            target["idx"] = torch.tensor([list(map(lambda t: torch.arange(t.size(0)) if isinstance(t, torch.Tensor) else torch.tensor([]), target["labels"]))]) if len(t.shape) > 0 #? this or below? 
