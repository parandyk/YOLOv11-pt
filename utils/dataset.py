...
class Dataset(data.Dataset):
    ...
    def collate_fn(batch): #original
        ...
        targets["label"] = list(map(lambda t: t if isinstance(t, torch.Tensor) elif (isinstance(t, float) and math.isnan(t)) torch.tensor([]), targets["labels"]))
        #targets["label"] = list(map(lambda t: t if isinstance(t, torch.Tensor) else torch.tensor([]), targets["labels"]))
        #print(targets["labels"])
        targets["boxes"] = list(map(lambda t: t if isinstance(t, torch.Tensor) elif (isinstance(t, float) and math.isnan(t)) torch.tensor([]), targets["boxes"]))
